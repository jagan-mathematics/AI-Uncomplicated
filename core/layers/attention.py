...
class RopeAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, output_attentions=False):
        ...
        query_state = self.query_projection(input_tensor)  # [B * S * D]
        key_state = self.key_projection(input_tensor)  # [B * S * D]
        value_state = self.value_projection(input_tensor)  # [B * S * D]

        query_state = query_state.view(b_size, seq_len, self.num_heads, self.head_dim).transpose(1,
                                                                                                 2)  # [B * H * S * D]
        key_state = key_state.view(b_size, seq_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B * H * S * D]
        value_state = value_state.view(b_size, seq_len, self.num_heads, self.head_dim).transpose(1,
                                                                                                 2)  # [B * H * S * D]
