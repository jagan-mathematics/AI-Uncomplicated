...
class PackedCausalTransformerGenerator:
    ...
    def clear_cache(self, offset):
        for module in self.model.modules():
            if isinstance(module, MultiTypeAttentionBlock):
    ...
    def generate_next_token(self, current_token):
        ...
        out = self.model.forward(
            current_token,
            token_idx=self.current_tok_id,  # n_seqs
            attention_mask=mask,
            attn_impl="sdpa",
        )
    ...
    def generate(self, prompts):
        # Tokenize
        prompts = [
            self.tokenizer.encode(p, return_type=None)["input_ids"][0] for p in prompts
        ]
        # Truncate
        max_seqlen = (
            self.max_tokens
        ...
        for batch in it:
            ...
            all_tokens = sample_tokens(
                prompt_logits["logits"], self.temperature, self.top_p, self.top_k
            )
            ...
            for i in range(1, self.max_gen_len):
                ...
                next_token = sample_tokens(
                    next_logits["logits"].clone(), self.temperature, self.top_p, self.top_k
                )
                ...
                for seq_id, tok in enumerate(next_token.squeeze(0).tolist()):
                    if not is_done[seq_id]:
                        ...
                        is_done[seq_id] = (
                            contains_end_string or tok == self.tokenizer.end_token_idx
                        )
                if all(is_done):
                    break
            ...
            for p, logit in zip(
                batch, prompt_logits["logits"].squeeze(0).split(lengths.tolist())
            ):
                x = logit[:-1]
